...
contract("DataMarket", accounts => {
    it("create new data item correctly", async ()=> {
        ...
        await marketInstance.create(encrypted_data_hash, prime1, prime2, n, e, value);

        const data_item_id = 1;
        await marketInstance.getCommodityInfo(data_item_id, {from: account1}).then(
            function(result) {
                var id = result[0];
                assert.equal(id.toNumber(), 1, "err item id");
                var value = result[1];
    ...
    it("participate and refund", async() => {
        ...
        await marketInstance.participate(data_item_id, {from: account2, value: received_value});
        ...
        await marketInstance.refund(data_item_id, {from: account2});
        await marketInstance.getCommodityInfo(data_item_id, {from : account1}).then(
            function(result) {
                var received_value = result[6];
                assert.equal(received_value.toNumber(), 0, "received_value error");
    ...
    it("participate and withdraw", async ()=>{
        ...
        // create a new data item
        await marketInstance.create(encrypted_data_hash, prime1, prime2, n, e, value, {from : account1});

        const data_item_id = 1;
        const received_value = 200000000;

        marketInstance.participate(data_item_id, {from: account2, value: received_value});
        // check the commodity received value
        marketInstance.getCommodityInfo(data_item_id, {from : account1}).then(
            function(result) {
        ...
        marketInstance.withdraw(data_item_id, n, sign, {from: account3});

        marketInstance.getCommodityInfo(data_item_id, {from : account1}).then(
            function(result) {
                var data_status = result[4];
